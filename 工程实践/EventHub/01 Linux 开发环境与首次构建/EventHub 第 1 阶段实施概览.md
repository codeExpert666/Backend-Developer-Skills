...
| 通用知识 | 换项目、换机器后仍成立的原理和操作方法 | UTM、Linux、SSH、Terminal、Git、Go、Docker、Java 专题 |
...
| 9      | Git 与基础命令行工具专题                  | Git、curl、证书和基础 PATH 可验证                          |
| 10（可选） | [[现代终端环境搭建概览]]                  | Mac mini 与 Ubuntu Server 的交互环境已按需配置；未配置不阻塞第 1 阶段 |
| 11     | Go、Java、Docker 专题               | 项目工具版本满足约束，daemon 与 CLI 可验证                      |
| 12     | [[Git 仓库跨机器迁移与工作区保留]]           | 能按源仓库状态选择安全迁移方式                                  |
| 13     | [[EventHub 仓库迁移与首次质量门禁]]        | 两个项目在 Linux 完成当前 revision 的门禁                    |
| 14     | [[UTM 虚拟机快照、备份与恢复]]             | 已建立环境与源码的分层恢复基线                                  |
| 15     | [[EventHub 第 1 阶段验收清单]]         | 证据齐全，可以停止本阶段                                     |

已有工具专题继续作为安装与排障事实源：

- Git：[[Git 安装与初始配置概览]]、[[Ubuntu 从零安装 Git]]、[[Git 常用配置与本地验证]]、[[Git 凭据、SSH 与常见问题排查]]。
- Terminal：[[现代终端环境搭建概览]]、[[macOS 从零搭建现代终端环境]]、[[Ubuntu 从零搭建现代终端环境]]。
- Go：[[Ubuntu 安装 Go]]。
...
#### 5.5.1 安装 Git 与基础命令行工具

先完成 Git、curl、CA 证书和项目后续步骤需要的基础命令行工具，再继续个人终端配置或项目工具链安装。Git 的安装、身份配置和凭据边界分别由 [[Ubuntu 从零安装 Git]]、[[Git 常用配置与本地验证]] 与 [[Git 凭据、SSH 与常见问题排查]] 负责。

这一层只建立可靠的命令执行、下载和仓库访问能力。日常 Git 操作不依赖 `sudo`，基础 PATH 不依赖交互插件，Git 托管平台凭据也不写入共享 Shell 配置。

#### 5.5.2 可选：建立现代终端交互环境

现代终端环境适合作为第 1 阶段的个人开发体验增强，但不是 EventHub 构建、测试或阶段退出的必要条件。先阅读 [[现代终端环境搭建概览]]，再按实际运行位置选择平台笔记：

| 位置 | 本阶段采用的职责边界 |
| --- | --- |
| Mac mini | 按 [[macOS 从零搭建现代终端环境]] 配置 Ghostty 与本地 Shell 工具，作为 SSH 的本地图形终端和交互入口 |
| Ubuntu Server | 按 [[Ubuntu 从零搭建现代终端环境]] 配置 Zsh、Antidote、Starship、Atuin、zoxide 与 fzf；无图形桌面时不安装 Ghostty |
| EventHub 项目 | 构建、测试、质量门禁和恢复命令不依赖别名、插件、提示符或个人 Shell 配置 |

执行时遵循以下顺序：

1. 先证明 UTM 控制台、Bash 基础环境、SSH 密钥登录和基础 PATH 正常。
2. 备份现有 Shell 配置；修改远端登录 Shell 前保留一个已登录 SSH 会话和 UTM 控制台恢复路径。
3. 分别完成 Mac mini 本地终端与 Ubuntu Server 命令行组件的最小配置。
4. 新开交互 SSH 会话，并验证 SSH 非交互命令仍能找到项目需要的工具。
5. 基础交互与恢复路径稳定后，再按需导入历史、启用 Atuin 同步或增加主题和插件。

共享配置不保存代理凭据、令牌、真实地址、命令历史或仅属于某台机器的私有路径；机器差异放入不提交的本地配置。不要删除 Bash、修改 `root` 的登录 Shell，也不要把个人别名或函数写成 EventHub 文档、脚本或 CI 的前置条件。
...
1. Go 工具链。
2. JDK 与项目实际需要的 Maven 路线。
3. Docker Engine、Docker CLI、Compose plugin。
4. 当前 revision 额外声明的工具，例如 Node.js 或静态分析器。
