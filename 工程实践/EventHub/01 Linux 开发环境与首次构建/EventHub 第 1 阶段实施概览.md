...
| 7      | [[Ubuntu Server 初始化与基础安全]]      | 用户、时间、更新、服务、防火墙和目录符合预期                           |
...
创建流程由 [[使用 UTM 创建 Ubuntu Server 虚拟机]] 负责。Ubuntu 首次从虚拟磁盘启动后，先在控制台依次完成 [[Shell 命令结构、类型与帮助系统]]、[[Shell 路径、变量、引用与展开]]、[[Shell 标准流、管道、重定向与退出状态]] 和 [[Shell 脚本阅读基础]] 的安全练习，再进入 [[Ubuntu Server 初始化与基础安全]]。不要求背完参数，但必须先能读懂初始化笔记中的变量、引用、管道、重定向、条件和退出状态。
