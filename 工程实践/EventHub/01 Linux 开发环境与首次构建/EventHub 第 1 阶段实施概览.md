...
| 2 | [[UTM 虚拟机网络与资源规划]] | 根据宿主资源选择网络、vCPU、内存和磁盘 |
| 3 | [[EventHub 第 1 阶段环境与版本基线]] | 记录本阶段候选配置和项目约束 |
| 4 | [[使用 UTM 创建 Ubuntu Server 虚拟机]] | Ubuntu Server 能从虚拟磁盘启动并联网 |
| 5 | [[Ubuntu Server 初始化与基础安全]] | 用户、时间、更新、服务、防火墙和目录符合预期 |
| 6 | [[OpenSSH 连接、密钥与主机指纹]] | Mac mini 能用密钥登录，新会话仍正常 |
| 7 | [[Linux 开发工作区与本地文件系统规划]] | 项目长期位于 Ubuntu 本地 `$HOME/src` |
| 8 | Git、Go、Java、Docker 专题 | 工具版本满足项目约束，daemon 与 CLI 可验证 |
| 9 | [[Git 仓库跨机器迁移与工作区保留]] | 能按源仓库状态选择安全迁移方式 |
| 10 | [[EventHub 仓库迁移与首次质量门禁]] | 两个项目在 Linux 完成当前 revision 的门禁 |
| 11 | [[UTM 虚拟机快照、备份与恢复]] | 已建立环境与源码的分层恢复基线 |
| 12 | [[EventHub 第 1 阶段验收清单]] | 证据齐全，可以停止本阶段 |
