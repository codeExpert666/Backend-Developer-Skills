...
| 工程化交付 | 开发环境出站访问、原生部署、Nginx 反向代理与 TLS、容器化、CI/CD、回滚和可观测性 | [[EventHub 工程化交付路线图]] | 按工程化第 1～6 阶段顺序验收 |
| AI 应用开发 | 内容生成、语义检索、RAG、工具调用、受控 Agent、评测与安全 | [[EventHub AI 应用实践路线图]] | 通用能力可独立学习，项目接入受业务门槛约束 |

工程化交付中有两个方向相反的“代理”：出站代理是 Git、Go、Maven 或 Docker 等客户端先连接代理服务，再访问外部目标，通用知识见 [[Linux 开发环境出站代理配置与分层排查]]；Nginx 反向代理是对外接收请求后转发给 EventHub 服务，属于工程化第 3 阶段。TLS 是保护网络传输的安全协议；CI/CD 在本路线中指持续集成与持续交付流程。
