...
通用的 clone、bundle、patch 与 `rsync` 方法见 [[Git 仓库跨机器迁移与工作区保留]]。本文不再复制完整迁移教程，只记录 EventHub 工程化第 1 阶段需要作出的选择、目标目录、项目专属命令和验收边界。
...
>
> `git`、`go`、`mvn`（或 `./mvnw`）与 `docker`（含 `docker compose`）都是各自项目或工具链的 CLI，不是 Linux 基础命令或 Shell 语法。Shell 负责解析整行并启动它们；具体子命令和参数应回到对应专题、当前 revision 的工程入口或该 CLI 帮助中核对。
...
准备检查发现的动态 Git 状态见 [[2026-07-16 工程化第 1 阶段准备检查记录]]。真正迁移前必须重查，历史记录不能替代当前事实。
...
本手册中含 `exit` 的交互代码块会显式放进圆括号创建的子 Shell；前置条件失败时只停止当前代码块，不会退出宿主机或虚拟机的登录 Shell。
...
Go 安装见 [[Ubuntu 安装 Go]]，精确版本见 [[EventHub 工程化第 1 阶段环境与版本基线]]。
...
详细退出条件由 [[EventHub 工程化第 1 阶段验收清单]] 统一维护。本篇至少应完成：
...
验证通过后，继续 [[UTM 虚拟机快照、备份与恢复]]，再回到 [[EventHub 工程化第 1 阶段验收清单]] 收口。
