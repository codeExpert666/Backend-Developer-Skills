...
本文是 [[EventHub 项目实践总览]] 下的工程化交付维度入口，负责让 EventHub 从本地 Linux 开发环境逐步演进为可部署、可发布、可回滚、可观测的系统，并维护六个工程化阶段的依赖、入口和完成状态。

项目级路线关系、阶段称谓和内容分层见 [[EventHub 项目实践总览]]。Linux、虚拟化、Git、Go、Java、Docker、MySQL 等技术原理继续沉淀在对应主题目录；本目录只记录这些能力如何在 EventHub 的交付链路中组合、验证和演进。
...
| --- | --- | --- | --- |
| 工程化第 1 阶段 | 建立 UTM Linux 开发机，明确出站访问路径，两个仓库通过完整质量门禁 | 可恢复的开发虚拟机、Linux 工具链、出站访问与代理决策基线、两仓库首次构建证据 | 云部署、正式发布流水线、生产监控 |
...
工程化第 1 阶段的出站代理服务于开发工具主动访问外部仓库，通用原理见 [[Linux 开发环境出站代理配置与分层排查]]；工程化第 3 阶段的 Nginx 反向代理服务于外部客户端访问 EventHub。两者方向、发起者和配置位置不同，不共用一套配置。
...
AI 应用开发是与本路线并列的项目实践维度，不是工程化第 7 阶段。两条路线的关系见 [[EventHub 项目实践总览]]；AI 业务门槛、能力顺序和共享评测资产见 [[EventHub AI 应用实践路线图]]。AI 功能进入部署阶段后，继续复用本路线的发布、回滚和可观测能力。
...
| 工程化第 4 阶段 | 未开始 | 依赖工程化第 3 阶段明确数据与 Nginx 反向代理边界 |
...
- 出站访问默认先验证直连；只有直连受限或组织策略明确要求时，才按发起请求的进程配置可信代理。
- 代理地址、端口和凭据是机器或组织配置，不写入 EventHub 仓库或公开执行记录；不用关闭 TLS 校验代替证书与信任链修复。
