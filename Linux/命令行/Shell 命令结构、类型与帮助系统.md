...
在 Ubuntu Server 中，通过本地控制台或 SSH 登录后常会进入 Bash。Bash 先解析变量、引号、管道和重定向，再决定命令名对应什么并执行它。初学阶段先记住“终端是交互入口，Shell 是解释器”；终端模拟器、TTY、PTY、本地 Shell 与远端 Shell 的完整关系见 [[终端、TTY、PTY 与 Shell 的关系]]。本文先继续学习命令结构，不在第一个示例中引入进程和终端设备诊断语法。
