...
本节只解释路径怎样定位对象，不解释 `/etc`、`/usr`、`/var` 等目录为什么存在、通常由谁管理或数据能否跨重启保留。理解完绝对路径与相对路径后，可继续阅读 [[Linux 文件系统层次与常见目录规范]] 建立目录职责地图。
...
练习后尝试先预测、再运行。下面的 `test -n` 检查变量展开结果非空，`test -d` 检查路径指向目录；完整条件骨架见 [[Shell 脚本阅读基础#6. 使用 test 表达条件|test 条件判断]]。
