...
这里的代理是指 APT 进程先连接代理服务，再由代理访问软件源。APT 通常经 `sudo` 以 root 身份运行，不能把当前用户 Shell 中的代理变量当作它必然读取的配置。遇到代理错误时，先读取 APT 已生效配置，再做一次性验证；完整步骤见 [[Linux 开发环境出站代理配置与分层排查#8. APT：系统软件包下载是独立消费者]]。
...
以下练习只查询当前索引和已安装数据库，不安装、升级或删除任何软件，也不需要 `sudo`。末尾的 `test -f` 用退出状态报告重启标记是否为普通文件，条件骨架见 [[Shell 脚本阅读基础#6. 使用 test 表达条件|test 条件判断]]。
