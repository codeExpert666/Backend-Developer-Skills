...
成功时，第一条 `find` 应显示 `project/`、`project/docs/`、`readme.txt` 和 `readme.old.txt`，但不再显示 `readme.backup.txt`；第二条 `find` 同时应用类型、名称和时间测试，应只显示刚创建的两个 `.txt` 文件。`du` 与 `df` 的具体数字取决于运行环境，不要求相同。若出现 `FAIL`，先使用输出的 `lab=` 路径检查最后成功到哪一步，不要立即运行清理阶段。
...
9. `du -sh /var/log` 与 `df -hT /var/log` 分别回答什么问题？
10. 为什么命令退出状态为 `0`，仍不能完全替代对操作后状态的检查？
...
- [GNU Coreutils Manual](https://www.gnu.org/software/coreutils/manual/coreutils.html)（2026-07-19）
