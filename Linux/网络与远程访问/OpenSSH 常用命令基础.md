...
`-A` 不覆盖已经存在的默认主机密钥，但每把新生成的密钥都会产生新的指纹；它无法找回已经丢失的原密钥，也不会处理 `sshd_config` 中指向其他位置的自定义 `HostKey`。对于已经被客户端信任的服务器，应优先恢复原主机密钥；只有新服务器或确认原密钥无法恢复并接受相应指纹变化时，才使用该命令。完整的诊断、服务启动、指纹核对和客户端恢复流程见 [[OpenSSH 密钥登录、服务端配置与排查#服务端缺少可用主机密钥]]。
