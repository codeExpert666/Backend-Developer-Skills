...
输出可能包含代理地址或凭据，分享前必须脱敏。`--show-origin` 用来指出配置来自哪个文件，便于区分系统、用户和当前仓库配置。

### 9.2 对 HTTPS 远程做一次性验证

`git -c` 只为本次 Git 进程增加配置，不写入文件。
...
git -c http.proxy="$PROXY_URL" ls-remote origin HEAD
)
```

`git ls-remote` 读取远程引用，不修改工作区。成功只能证明当前凭据和网络路径足以读取该远程；推送权限仍需单独验证。

### 9.3 确需持久化时缩小范围

当前仓库的 `remote.origin.proxy` 写入 `.git/config`，不会进入普通项目提交，比无条件写入全局 `http.proxy` 更窄。下面的代码块会先重新读取代理 URL，并确认 `origin` 是 HTTPS 远程，然后才修改当前仓库：
