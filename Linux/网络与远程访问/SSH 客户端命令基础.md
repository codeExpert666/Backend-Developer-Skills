...
本篇只解释 `ssh` 客户端的常用命令骨架、本地与远端的执行边界、身份选择、客户端配置和基础调试。SSH 连接边界、服务端安装、指纹核对、用户密钥登记、`authorized_keys` 与认证策略见 [[OpenSSH 密钥登录、服务端配置与排查]]；连接前的 TCP 探测见 [[TCP 端口连通性测试与 nc 命令基础]]；变量、引号和重定向见 [[Shell 路径、变量、引用与展开]] 与 [[Shell 标准流、管道、重定向与退出状态]]。
...
本地 Shell 会先展开双引号中的变量，例如把它变成一个 `linux-user@server.example.com` 参数。`@` 只是远程用户名与目标主机的分隔符；双引号保证展开结果作为一个整体交给 `ssh`。实际脚本仍应像 [[OpenSSH 密钥登录、服务端配置与排查]] 那样先拒绝空值和以连字符开头的目标，避免把输入误认成选项。
