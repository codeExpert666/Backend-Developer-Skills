...
`ss` 是 Linux 网络工具集 iproute2 提供的套接字查看工具。网络命名空间（network namespace）为一组进程提供相对独立的网络接口、路由和套接字视图。`ss` 默认只读取命令所在网络命名空间中的套接字；在没有容器等网络隔离的普通主机上，这通常就是当前 Linux 主机的默认网络命名空间。

`ss` 能够回答当前可见套接字的协议、状态、本地端点、对端端点以及进程关联，但不会：
...
如果要在当前网络命名空间中查找与服务端口 `8080` 有关的所有可见 TCP 套接字记录，可以同时匹配两侧：

```bash
ss -antp '( sport = :8080 or dport = :8080 )'
```

这条命令不会到对端主机查询套接字。它只从当前网络命名空间中已经可见的记录里，保留本地端口或对端端口为 `8080` 的行。假设客户端使用临时端口 `53000` 连接同一网络命名空间中的 `127.0.0.1:8080`，可能看到以下三类记录：

| 当前可见的套接字记录 | 匹配条件 | 含义 |
| --- | --- | --- |
| `LISTEN 127.0.0.1:8080 → 0.0.0.0:*` | `sport = :8080` | 服务端监听套接字 |
| `ESTAB 127.0.0.1:8080 → 127.0.0.1:53000` | `sport = :8080` | 服务端已连接套接字 |
| `ESTAB 127.0.0.1:53000 → 127.0.0.1:8080` | `dport = :8080` | 客户端已连接套接字 |

后两行是同一条 TCP 连接两侧各自的套接字记录。只有客户端套接字与 `ss` 命令位于同一网络命名空间，客户端连接行才会出现在这次查询中。如果客户端位于远端主机或另一个网络命名空间，服务端执行的 `ss` 仍能在服务端连接行中显示对端端点，但不能读取客户端自己的套接字记录。
...
本实验的客户端、服务端和 `ss` 命令都运行在同一个网络命名空间中，因此通常可以同时看到：
...
三者的端口一致，仍只完成了服务端检查。最终还需要从真实客户端连接实际地址和端口，并继续完成 SSH 主机身份和用户身份验证，详见 [[OpenSSH 密钥登录、服务端配置与排查#2. 在服务端准备 sshd]] 与 [[Linux 主机防火墙与 UFW 基础#6.1 确认 SSH 管理入口并选择规则写法]]。
...
第 5.1 节所述的观察边界在容器等网络隔离场景尤其重要：

- 同一台物理主机上的进程仍可能位于不同网络命名空间；宿主机上的 `ss` 不保证列出容器网络命名空间内部的套接字。
- 一行中的对端端点只描述当前套接字正在与谁通信，不表示 `ss` 已经读取对端自己的套接字记录。
- `sudo` 可以帮助读取更多进程关联信息，但不会自动合并不同网络命名空间的套接字视图。
