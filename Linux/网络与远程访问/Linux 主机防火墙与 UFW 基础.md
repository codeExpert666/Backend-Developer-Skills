...
UFW 会跟踪连接状态。主机向外发起连接时，UFW 会记录这条连接；对方随后传回本机的数据会被识别为这条连接的响应，而不是从外部发起的新连接。
...
5. 不一致时停止，不要靠猜测添加规则；按 [[OpenSSH 密钥登录、服务端配置与排查#2. 在服务端准备 sshd]] 先确认实际激活路径和监听端口。
