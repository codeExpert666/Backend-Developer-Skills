...
首次阅读时，只需先看清这些状态分别保存在哪里、由谁携带以及何时变化，不必一次记住 NSS、sudo 认证状态或 `umask` 的全部细节；后文会在相关命令出现时逐项展开。
...
`r`、`w`、`x` 使用相同的字母表示，但实际允许的操作取决于对象类型。先理解这些权限位对文件和目录分别意味着什么，再把它们转换成数字：
