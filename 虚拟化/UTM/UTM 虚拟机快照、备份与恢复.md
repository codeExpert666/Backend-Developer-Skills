...
> [!info] 资料核对日期
> 本文涉及 UTM Actions、控制方式和磁盘操作的信息于 **2026-07-17** 根据官方文档核对。UTM 的快照、Export、Duplicate 或复制能力会随版本、虚拟化后端和虚拟磁盘格式变化，操作前应在当前版本中重新确认。
...
虚拟磁盘的标称上限、当前逻辑大小和实际占用并不相同。稀疏磁盘复制到不支持 clone 或稀疏文件的目标文件系统时，可能占用显著更多空间。复制完成后，应再次检查目标卷空间，不要立即删除原 VM。
