...
本文展示的软件包树镜像的是**默认 XDG 布局**：配置源中的 `.config/...` 经 `--target="$HOME"` 部署为 `$HOME/.config/...`。仅把 `XDG_CONFIG_HOME` 改成其他目录，不会让 Stow 自动重写仓库源或目标，应用与链接反而会分流。四条标准搭建主线因此只接受变量未设置、为空或等于默认值；非默认布局必须先确定新的仓库镜像结构、Stow target、旧文件迁移与回退，再部署，不能直接照抄本文命令。
...
Atuin 安装器还可能在配置目录生成本机 `atuin-receipt.json`，日志则由受管配置定向到 XDG state。使用 `--no-folding` 时，配置目录保持为真实目录，受管 `config.toml` 链接可以与这类本机文件并存；它们都不应反向进入 Git 源树。
...
| Atuin 安装回执与日志 | 不跟踪 | 本机安装元数据与持续变化的状态 |
