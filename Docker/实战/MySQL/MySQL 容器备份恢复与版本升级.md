...
`test -s` 只确认备份路径存在且文件大小大于 `0`，不会证明 SQL 完整或可恢复；该条件的退出状态与其他常用写法见 [[Shell 脚本阅读基础#6. 使用 test 表达条件|test 条件判断]]。

参数含义：

| 参数 | 作用 |
| --- | --- |
| `-T` | 关闭 Compose 的 [[终端、TTY、PTY 与 Shell 的关系\|伪终端（PTY）]] 分配，避免污染重定向输出 |
